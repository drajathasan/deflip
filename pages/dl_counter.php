...
<div class="sub_section">
    <form method="get" action="<?= $iframeUrl ?>" target="reportView">
...
<iframe name="reportView" id="reportView" src="<?= $iframeUrl ?>" frameborder="0" style="width: 100%; height: 500px;"></iframe>
<?php
} else if ($reportView && isset($_GET['detailList'])) {
    unset($_GET['reportView']);
   include __DIR__ . '/dl_detail.php';
} else {
    ob_start();
    // create datagrid
...
//    $outer_criteria = 'b.biblio_id > 0 ';
    ...
    function callbackLinkDetail($obj_db, $rec_d)
    {
        ...
        global $iframeUrl;
        return '<a onclick="top.$(\'#mainContent\').simbioAJAX(\''.$iframeUrl.'&fid='.$rec_d[4].'&detailList=yes\')" title="'.__('Detail').'"><i class="fa fa-search" aria-hidden="true"></i>';
    }
